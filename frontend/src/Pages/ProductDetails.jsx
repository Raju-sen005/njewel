...
const ProductHighlightSection = ({ product, multiproduct }) => {
    ...
    if (!name) {
        return <p>No product data found.</p>; // fallback
    }
    ...
    const checkPincode = () => {
        alert('Pincode check functionality to be implemented');
    };
        ...
        <section className="product-section px-6 py-12">
            <div className="flex flex-col md:flex-row w-full bg-[#F6F4F0]">
                ...
                <div className="w-full md:w-1/2 p-4 bg-[#E9E2D8]">
                    ...
                    {/* <p className="text-lg">{description || "No description provided."}</p> */}
                    {/* </div> */}
                    {/* Main Image Carousel */}
                        ...
                        <Slider {...mainSettings}>
                            {product?.images?.map((img, index) => (
                                <div key={index} className="px-2">
                                    ...
                                    <img src={image} alt={name} className="w-full max-w-md mx-auto mb-4" />
                                    </div>
                                </div>
                            ))}
                        </Slider>
                ...
                <div className="w-full md:w-1/2 flex flex-col px-6 md:px-10 py-8 bg-[#F6F4F0]">
                    ...
                    {/* Gemstone Selection */}
                    {multiproduct?.filter(m => m?.gemstone).length > 0 && (
                        <div className="mb-4 mt-6">
                            <h3 className="text-sm font-semibold mb-2">Gemstone Type</h3>
                            <div className="flex flex-wrap gap-2">
                                {multiproduct
                                    .filter(metal => metal?.gemstone) // ✅ Only items with gemstone
                                    .map((metal, index) => (
                                        <div key={index} className="flex flex-col items-center space-y-1">
                                            <button
                                                onClick={() => navigate(`/product/${metal?.productId}/${metal?.id}`)}
                                                className={`flex-shrink-0 w-16 h-16 border-2 ${selectedGemId === metal?.id ? 'border-[#AA8265]' : 'border-gray-200'
