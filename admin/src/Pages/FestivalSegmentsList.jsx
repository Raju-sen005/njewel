...
function FestivalSegmentsList() {
    const baseURL = import.meta.env.VITE_BACKEND_URL


    const validationSchema = Yup.object().shape({
        festival: Yup.string().required('Festival is required'),
    ...
    const fetchCustomers = async () => {
        try {
            ...
            const res = await axios.get(`${baseURL}/festival`, { headers });
    ...
    const handleSubmit = async (values, { resetForm }) => {
        ...
        try {
            if (editingcustomer) {
                await axios.put(`${baseURL}/festival/${editingcustomer.id}`, payload, { headers });
            } else {
                await axios.post(`${baseURL}/festival`, payload, { headers });
            }
    ...
    const handleDelete = async (customerId) => {
        ...
        try {
            await axios.delete(`${baseURL}/festival/${customerId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
                ...
                <div className="lg:ml-64 ml-0">
                    ...
                    <main className="p-4 lg:p-6">
                        <div className="flex justify-between p-4">
                            <h1 className="text-2xl font-semibold">Festival Management</h1>
                            <button
                                    ...
                                    {Customers.length > 0 ? (
                                        Customers.map((customer, index) => (
                                            <tr key={customer.id} className="border-b hover:bg-gray-100 dark:hover:bg-gray-700">
                                                ...
                                                <td className="p-4">{customer.festival}</td>
                        ...
                        {showForm && (
                            ...
                            <Formik
                                initialValues={{
                                    festival: editingcustomer?.festival || '',
                                ...
                                {() => (
                                    <Form className="p-4 mt-6 bg-white shadow rounded-md max-w-md mx-auto">
                                        <h2 className="text-xl font-semibold mb-4">
                                            {editingcustomer ? 'Edit Festival' : 'Create Festival'}
                                        </h2>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            >
                                                {editingcustomer ? 'Update Festival' : 'Add Festival'}
                                            </button>
                                            <button
                                                type="button"
...
export default FestivalSegmentsList