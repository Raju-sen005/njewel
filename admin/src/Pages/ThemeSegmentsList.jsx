...
function ThemeSegmentsList() {
    ...
    const handleSubmit = async (values, { resetForm }) => {
        ...
        try {
            if (editingcustomer) {
                await axios.put(`${baseURL}/theme/${editingcustomer.id}`, payload, { headers });
            } else {
                await axios.post(`${baseURL}/theme`, payload, { headers });
            }
    ...
    const handleDelete = async (customerId) => {
        ...
        try {
            await axios.delete(`${baseURL}/theme/${customerId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
                                    ...
                                    {Customers.length > 0 ? (
                                        Customers.map((customer, index) => (
                                            <tr key={customer.id} className="border-b hover:bg-gray-100 dark:hover:bg-gray-700">
                                                ...
                                                <td className="p-4">{customer.theme}</td>
                                ...
                                {() => (
                                    <Form className="p-4 mt-6 bg-white shadow rounded-md max-w-md mx-auto">
                                        <h2 className="text-xl font-semibold mb-4">
                                            {editingcustomer ? 'Edit Theme' : 'Create Theme'}
                                        </h2>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            >
                                                {editingcustomer ? 'Update Theme' : 'Add Theme'}
                                            </button>
                                            <button
                                                type="button"
